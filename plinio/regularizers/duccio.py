...
class DUCCIO():
    ...
    def __call__(self, model: DNAS, epoch: int = 1, n_epochs: int = 1) -> torch.Tensor:
        ...
        if self.final_strengths is None:
            self.final_strengths = (torch.min(torch.tensor(0.0, device=model_device), self.task_loss / (model.get_cost(n) - t)) for n, t in self.targets.items())

        cost = torch.tensor(0.0).to(model_device)
        for (cost_name, target), strength in zip(self.targets.items(), self.final_strengths):
            ...
            cost += (eff_strength * torch.max(torch.tensor(0.0).to(model_device), model.get_cost(cost_name) - target))
        return cost
